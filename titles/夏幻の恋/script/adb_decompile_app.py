...
from script.nbda.adbsrc import render_ir_adbsrc
from script.nbda.decompile import parse_adb, parse_adb_ir, validate_magic


def build_parser(
    *,
    description: str = "Decompile NBDA script.",
    default_mode: str = "ir",
    ...
    parser.add_argument(
        ...
        help=f"Intermediate mode. ir: instruction-level structure. raw: byte-exact structure. Default: {default_mode}.",
    )
    parser.add_argument(
        "--output-format",
        choices=["json", "adbsrc"],
...
def iter_adb_files(input_path: Path) -> list[Path]:
...
def default_output_for_file(input_file: Path, output_format: str) -> Path:
    if output_format == "json":
        return input_file.with_suffix(input_file.suffix + ".json")
    return input_file.with_suffix(".adbsrc")


def default_output_for_relpath(rel: Path, output_format: str) -> Path:
    if output_format == "json":
...
def resolve_output(
    ...
    if input_path.is_file():
        out = Path(output_arg) if output_arg else default_output_for_file(input_path, output_format)
    ...
    for adb_file in adb_files:
        ...
        out_file = out_root / default_output_for_relpath(rel, output_format)
        pairs.append((adb_file, out_file))
    return pairs


def serialize_doc(doc: dict, output_format: str) -> str:
    if output_format == "json":
