...
def _slot_bytes_ir(slot: dict) -> bytes:
    ...
    if opcode == 0x600 and slot.get("editable_text", False) and slot.get("text_role") == "speaker_name":
        speaker_name = str(slot.get("speaker_name", slot.get("text", "")))
        name_raw = speaker_name.encode("utf-16le")
        name_units = list(struct.unpack(f"<{len(name_raw) // 2}H", name_raw)) if name_raw else []
        words = [0x600, 0x0000, len(name_units) + 1, *name_units, 0x0000]
