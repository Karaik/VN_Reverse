...
def _decode_slot(chunk: bytes) -> dict:
    ...
    # 0x0600 speaker-name variant:
    # [opcode][0][name_len_plus_terminator][name_u16...][0]
    if opcode == 0x0600 and len(words) >= 4:
        name_len_plus_term = words[2]
        if words[1] == 0 and name_len_plus_term >= 1 and len(words) == 3 + name_len_plus_term and words[-1] == 0:
            name_units = words[3:-1]
            name_raw = struct.pack(f"<{len(name_units)}H", *name_units) if name_units else b""
            try:
                speaker_name = name_raw.decode("utf-16le")
            except UnicodeDecodeError:
                speaker_name = None
            if speaker_name:
                slot["editable_text"] = True
                slot["text_role"] = "speaker_name"
                slot["speaker_name"] = speaker_name
                slot["text"] = speaker_name
                slot["speaker_name_len_u16"] = len(name_units)
                return slot
...
def parse_adb_ir(data: bytes) -> dict:
    ...
    for entry in entries:
        ...
        if text_role == "speaker_name":
            ...
            continue
        if text_role == "dialogue" and pending_speaker_name_slot_id is not None:
            speaker_slot = slot_by_id[pending_speaker_name_slot_id]
            speaker_name = str(speaker_slot.get("speaker_name", ""))
            entry["speaker_name_slot_id"] = pending_speaker_name_slot_id
            entry["speaker_name"] = speaker_name
            slot["speaker_name_slot_id"] = pending_speaker_name_slot_id
            slot["speaker_name"] = speaker_name
