...
from script.nbda.adbsrc import parse_adbsrc
...
def detect_format(path: Path, input_format: str) -> str:
...
def iter_source_files(input_path: Path, input_format: str) -> list[Path]:
...
def default_output_for_file(input_file: Path, source_format: str) -> Path:
...
def resolve_output(
    ...
    if input_path.is_file():
        ...
        out = Path(output_arg) if output_arg else default_output_for_file(input_path, fmt)
    ...
    for src in source_files:
        fmt = detect_format(src, input_format)
        rel = src.relative_to(input_path)
        out_file = out_root / default_output_for_file(rel, fmt)
        pairs.append((src, out_file, fmt))
    return pairs


def load_doc(path: Path, source_format: str) -> dict:
...
def main() -> int:
    ...
    source_files = iter_source_files(input_path, args.input_format)
    pairs = resolve_output(input_path, args.output, source_files, args.input_format)

    for src, output_path, source_format in pairs:
        doc = load_doc(src, source_format)
