...
from yuka.yks import normalize_text_encoding, parse_yks, render_ykssrc


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Decompile YKS script to JSON or YKSRC.")
    parser.add_argument("input", help="Input .yks file or directory.")
    parser.add_argument("output", nargs="?", help="Output file or directory.")
    parser.add_argument("--output-format", choices=["json", "ykssrc"], default="json", help="Output format.")
    parser.add_argument(
        "--text-encoding",
        default="cp932",
...
def _iter_inputs(input_path: Path) -> list[Path]:
    ...
    if input_path.is_dir():
        return sorted(p for p in input_path.rglob("*") if p.is_file() and p.suffix.lower() == ".yks")
...
def _default_output_for_file(path: Path, output_format: str) -> Path:
    ...
    return path.with_suffix(path.suffix + ".ykssrc")
...
def _resolve_output(input_path: Path, output_arg: str | None, files: list[Path], output_format: str) -> list[tuple[Path, Path]]:
    ...
    if not files:
        raise ValueError(f"No .yks files found in directory: {input_path}")
    suffix = "json" if output_format == "json" else "ykssrc"
...
def main() -> int:
    ...
    for src, dst in pairs:
        doc = parse_yks(src.read_bytes(), text_encoding=text_encoding)
        dst.parent.mkdir(parents=True, exist_ok=True)
        dst.write_text(_render(doc, args.output_format), encoding="utf-8")
        if args.verbose:
            print(f"[ok] {src} -> {dst} (encoding={text_encoding})")
    print(f"[done] decompiled {len(pairs)} file(s) to {args.output_format} (encoding={text_encoding}).")
    return 0
